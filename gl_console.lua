-- GL Console v1.4
-- Fix: ImGui.Begin tanpa p_open, OnValue debounce, autosave=false
...
-- Debounce: simpan value terakhir OnValue supaya tidak spam
local lastOnValue = {}
...
-- ============================================================
-- IMGUI DRAW
-- FIX: ImGui.Begin tanpa p_open (cukup 1 arg = judul)
-- FIX: Wrap di dalam if Console.open, jangan pass p_open
-- ============================================================
function OnDrawImGui(delta)
    if not Console.open then return end
    updateStats(delta)

    -- FIX: Begin sederhana tanpa p_open, tanpa flags
    -- Sesuai contoh di API docs: ImGui.Begin("Title")
    ImGui.Begin("GL Console##gc")

    -- STATS BAR
    ImGui.TextColored(C.stats, string.format(
        "[%s] XY:%d,%d | Gems:%d Dirt:%d",
    ...
    if ImGui.Button("X") then Console.logs = {} Console.logBuf = "" end
...
-- ============================================================
-- UI MODULE
-- FIX: autosave=false supaya GL tidak trigger OnValue tiap frame
-- FIX: OnValue pakai debounce - cek apakah value benar-benar berubah
-- ============================================================
...
ui:addDivider()
-- FIX: autosave=false (param ke-4)
ui:addToggle("Show Console", cfg:get("open", false), "console_open", false)

function OnValueConsole(type, name, value)
    -- FIX: Debounce - hanya proses jika value benar-benar berubah
    if lastOnValue[name] == value then return end
    lastOnValue[name] = value
...
LogSystem("GL Console v1.4 loaded!")
